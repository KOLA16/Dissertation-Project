...
def data_load(args):
    ...
    if args.trte == "val":  # train on 90% of source data and validate on the remaining 10% of source data
        dsize = len(txt_src)
        tr_size = int(0.9*dsize)
        tr_txt, te_txt = torch.utils.data.random_split(txt_src, [tr_size, dsize - tr_size])
    else:                   # train on 100% of source data and validate on 100% of source data (training set accuracy)
        # dsize = len(txt_src)
        # tr_size = int(0.9*dsize)
...
def train_source(args):
    ...
    for epoch in range(args.max_epoch):
        ...
        if args.dset == 'office-31':
            acc_s_te, mean_ent = cal_acc(dset_loaders['source_te'], netF, netB, netC, flag=False)
            ...
            acc_s_te, mean_ent = cal_acc(dset_loaders['source_te'], netF, netB, netC, flag=False)
            log_str = 'Task: {}, Iter:{}/{}; Accuracy = {:.2f}%'.format(args.name_src, iter_num, max_iter, acc_s_te)
        args.train_log.write(log_str + '\n')
        args.train_log.flush()
        print(log_str+'\n')
    ...
    # netB.eval()
    # netC.eval()
    # acc_s_te, mean_ent = cal_acc(dset_loaders['test'], netF, netB, netC, flag=False)
...
def test_target(args):
    ...
    acc, mean_ent = cal_acc(dset_loaders['test'], netF, netB, netC, flag=False)
    log_str = '\nTask: {}, Target Accuracy = {:.2f}%'.format(args.name, acc) + '\n'

    args.test_log.write(log_str)
    args.test_log.flush()
    print(log_str)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--mode', type=str, default='test', choices=['train+test', 'test'], help="train on source and"
                        "test on target or just test on target")
    ...
    if args.mode == "train+test":
        args.train_log = open(osp.join(args.output_dir_src, 'train_log_{}.txt'.format(args.seed)), 'w')
        args.train_log.write(print_args(args)+'\n')
        args.train_log.flush()
        train_source(args)

        args.test_log = open(osp.join(args.output_dir_src, 'test_{}_{}_log.txt'.format(names[args.t], args.seed)), 'w')
        test_target(args)
