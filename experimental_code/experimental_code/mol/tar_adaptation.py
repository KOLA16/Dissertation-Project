...
def init_memory_banks(model, device, loader, aad, indx_map, args):
    ...
    torch.save(aad.feature_bank, 'saved_features/{}_{}_tar_bbbp_features.pt'.format(args.model_num, args.gnn))
    torch.save(aad.true_labels_bank, 'saved_features/{}_{}_tar_bbbp_labels.pt'.format(args.model_num, args.gnn))
    torch.save(aad.score_bank, 'saved_features/{}_{}_tar_bbbp_softmax.pt'.format(args.model_num, args.gnn))
    plot_tsne(filename='{}_{}_tar_bbbp'.format(args.model_num, args.gnn), perplexity=50.0, early_exaggeration=12.0)
...
def eval(model, device, loader, evaluator):
    ...
    for step, batch in enumerate(tqdm(loader, desc="Iteration")):
        ...
        if batch_data.x.shape[0] == 1:
            ...
            y = torch.squeeze(F.one_hot(batch_data.y, num_classes=2))
...
def main():
    ...
    split_idx = dataset.get_idx_split("cluster")
    ...
    if args.gnn == 'gin':
        model = GNN(gnn_type='gin', num_tasks=dataset.num_classes, num_layer=args.num_layer, emb_dim=args.emb_dim,
                    drop_ratio=args.drop_ratio, virtual_node=False).to(device)
    elif args.gnn == 'gin-virtual':
        model = GNN(gnn_type='gin', num_tasks=dataset.num_classes, num_layer=args.num_layer, emb_dim=args.emb_dim,
                    drop_ratio=args.drop_ratio, virtual_node=True).to(device)
    elif args.gnn == 'gcn':
        model = GNN(gnn_type='gcn', num_tasks=dataset.num_classes, num_layer=args.num_layer, emb_dim=args.emb_dim,
                    drop_ratio=args.drop_ratio, virtual_node=False).to(device)
    ...
    model.load_state_dict(torch.load('trained_models/best_src_{}_bbbp_{}.pth'.format(args.gnn, args.model_num)))
    ...
    torch.save(best_model_adapt.state_dict(), 'trained_models/best_adapted_{}_bbbp_{}.pth'.format(args.gnn, args.model_num))
