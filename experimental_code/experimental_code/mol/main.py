...
def train(model, device, loader, optimizer):
    ...
    for step, batch in enumerate(tqdm(loader, desc="Iteration", disable=True)):
        ...
        if batch_data.x.shape[0] == 1 or batch_data.batch[-1] == 0:
            ...
            optimizer.zero_grad()
            loss = cls_criterion(pred, torch.squeeze(batch_data.y))
            loss.backward()
            optimizer.step()
...
def main():
    ...
    parser.add_argument('--split', type=str, default="cluster",
                        help='dataset split type')
    parser.add_argument('--model_num', type=int, default=1,
                        help='identification number for a trained model')
    ...
    torch.save(best_model_train.state_dict(), 'trained_models/best_src_{}_bbbp_{}.pth'.format(args.gnn, args.model_num))
