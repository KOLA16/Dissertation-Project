...
def plot_tsne(filename='src', perplexity=50.0, early_exaggeration=12.0):
    ...
    feature_bank_embed = TSNE(perplexity=perplexity, early_exaggeration=early_exaggeration, learning_rate="auto", init='pca')\
    ...
    p1 = sns.scatterplot(
        data=data_1, x=data_1[:, 0], y=data_1[:, 1], hue=data_1[:, 2], palette='nipy_spectral', legend='full', s=15
    )
    p1.legend_.remove()
    ...
    p2 = sns.scatterplot(
        data=data_2, x=data_2[:, 0], y=data_2[:, 1], hue=data_2[:, 2], palette='nipy_spectral', legend='full', s=10
    )
    p2.legend_.remove()
